   ...
   ;```````````````````````````````````````````````````````````````
   ;  Lives and fare metres
   ;
   dim _Player_Direction_X = n
   dim _Player_Direction_Y = o
   dim _Player_Acceleration_X = p
   dim _Player_Acceleration_Y = q
   dim _Number_Of_Lives = s
   dim _Remaining_Fare = t
   dim _Destination_Pad_Num = u
...
__Title_Screen_Setup
   ...
   XXX....XX..X.XX..XX.XX....XX....
   .XXXX..XXXX..XXXXXX.XX....XXXX..
   ....XX.XX....XX..XX.XX....XX....
   XXXXX..XX....XX..XX..XXXX..XXXXX
   ................................
...
__Main_Loop_Setup
   ...
   _Player_Direction_X = 0
   _Player_Direction_Y = 0

   ;TODO: random backgrounds

   ;data _Background_Colors
   ;$36, $86, $C2, $92
;end

   ; Get a random index
   ;const randomIndex = (rand&3) ; This will ensure the random index is between 0 and 3

   ; Set the background color
   ;COLUBK = _Background_Colors(randomIndex)
...
__Main_Loop
   ...
   ;  Handle acceleration
   ;
__Check_Acceleration_X_Right
   if _Player_Direction_X <> 2 then goto __Check_Acceleration_X_Left

   if player0x = 160 then goto __Check_Acceleration_X_Left

   player0x = player0x + _Player_Acceleration_X
   _Player_Acceleration_X = _Player_Acceleration_X - 1
   if _Player_Acceleration_X = 0 then _Player_Direction_X = 0

__Check_Acceleration_X_Left
   if _Player_Direction_X <> 1 then goto __Check_Acceleration_Y_Up 

   if player0x = 0 then goto __Check_Acceleration_Y_Up

   player0x = player0x - _Player_Acceleration_X
   _Player_Acceleration_X = _Player_Acceleration_X - 1
   if _Player_Acceleration_X = 0 then _Player_Direction_X = 0

__Check_Acceleration_Y_Up
   if _Player_Direction_Y <> 1 then goto __Check_Acceleration_Y_Down

   if player0y = 0 then goto __Check_Acceleration_Y_Down

   player0y = player0y - _Player_Acceleration_Y
   _Player_Acceleration_Y = _Player_Acceleration_Y - 1
   if _Player_Acceleration_Y = 0 then _Player_Direction_Y = 0

__Check_Acceleration_Y_Down
   if _Player_Direction_Y <> 2 then goto __Check_Joystick_Movement 
   
   if player0y = 120 then goto __Check_Acceleration_Y_Down

   player0y = player0y + _Player_Acceleration_Y
   _Player_Acceleration_Y = _Player_Acceleration_Y - 1
   if _Player_Acceleration_Y = 0 then _Player_Direction_Y = 0

__Check_Joystick_Movement
...
__AP_Move_Up
   ...
   _Player_Direction_Y = 1

   if _Player_Acceleration_Y < 30 then _Player_Acceleration_Y = _Player_Acceleration_Y + 1
...
__AP_Move_Left
   ...
   _Player_Direction_X = 1

   if _Player_Acceleration_X < 30 then _Player_Acceleration_X = _Player_Acceleration_X + 1
...
__Skip_Joy0_Right
   ...
   ;
   ;```````````````````````````````````````````````````````````````
__Sound_Thrusters_On

   if !joy0left && !joy0right && !joy0up && !joy0down then goto __Sound_Thrusters_Off

   AUDC0 = 2 ; Set sound waveform
   AUDF0 = 15 ; Set frequency
   AUDV0 = 10 ; Set volume

   goto __Autoplay_Inactivity

__Sound_Thrusters_Off
      AUDV0 = 0

__Autoplay_Inactivity
